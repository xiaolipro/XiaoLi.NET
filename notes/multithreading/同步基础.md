...
>Thread.Sleep(0)，Task.Delay(0)会立即释放当前的时间片，将 CPU 资源出让给其它线程。Thread.Yield()，Task.Yield()方法与其相同，但他只会出让给运行在相同处理器核心上的其它线程。
Sleep(0)和Yield在调整代码性能时偶尔有用，它也是一个很好的诊断工具，可以用于找出线程安全（thread safety）的问题。如果在你代码的任意位置插入Thread.Yield()会影响到程序，基本可以确定存在 bug。


编译器将代码转换为状态机，到达 await 时暂停执行等待后台作业完成时继续执行。从理论上讲，这是[异步的承诺模型](https://en.wikipedia.org/wiki/Futures_and_promises)的实现。）
```c#
